    ...
    public function onRun(): void {
        if ($this->time == 0) {
            ...
            if ($this->plugin->getBanAPI()->isBannedIP($player->getNetworkSession()->getIp())) {
                ...
                $player->kick((str_replace(["{time}", "{reason}", "{staff}"], [$formatTemp ,$reason, $staff], $this->plugin->getConfig()->getNested("ban-ip.connection-message"))));
